...
import { CheckCircle, ShoppingBag, ArrowLeft, Plus, Minus, Star, Truck, RefreshCw, Shield,X } from "lucide-react";
...
export default function ProductDetailsPage() {
  ...
  const handleBack = () => {
    // Check if there is a previous page in our own history
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-slate-100">
      ...
      <div className="min-h-screen bg-white">
        {/* 1. MOBILE FLOATING CLOSE BUTTON */}
        {/* This is much better for mobile "Quick View" users than a back link */}
        <button
          onClick={handleBack}
          className="fixed top-4 right-4 z-[60] p-2 bg-white/80 backdrop-blur-md rounded-full shadow-lg lg:hidden"
        >
        ...
        <div className="container mx-auto px-4 pt-4 pb-2">
          <button
            onClick={handleBack}
            className="inline-flex items-center gap-2 text-xs font-bold uppercase tracking-widest text-stone-400 hover:text-emerald-600 transition-colors group"
          >
              ...
              <div className="flex gap-3 overflow-x-auto pb-2 scrollbar-hide md:grid md:grid-cols-4">
                {images.map((img, idx) => (
                  <button
                    ...
                    className={`relative aspect-square rounded-xl md:rounded-2xl overflow-hidden transition-all duration-300 ${selectedImage === idx
              ...
              <div className="space-y-4 pt-2">
                <div className="flex flex-col sm:flex-row gap-4">
                  ...
                  <button
                    ...
                    disabled={isAdded}
                    className={`flex-1 flex items-center justify-center gap-3 px-8 py-4 font-bold text-base sm:text-lg rounded-xl transition-all duration-300 shadow-lg hover:shadow-xl transform hover:-translate-y-0.5 ${isAdded
